...
public class ServerTickHandler {
    ...
    public static void onTickInGame() {
        ...
        if (world != null && lastWorld != world) {
            lastWorld = world;
            // ((ServerCommandManager)FMLCommonHandler.instance().getMinecraftServerInstance().getCommandManager()).registerCommand(new
            // CommandWaveHeight());
            // ((ServerCommandManager)FMLCommonHandler.instance().getMinecraftServerInstance().getCommandManager()).registerCommand(new
            // CommandWeather());
        }
        ...
        for (int i = 0; i < worlds.length; i++) {
            ...
            if (wms != null) {
                lookupDimToWeatherMan.get(worlds[i].provider.dimensionId)
                    .tick();
            }
        }
        ...
        if (testRainRequest) {
            ...
            for (int i = 0; i < listMsgs.size(); i++) {
                ...
                // System.out.println("Weather2 side: " + listMsgs.get(i).key + " - modID: " +
                // listMsgs.get(i).getSender() + " - source: " + listMsgs.get(i).toString() + " - " +
                // listMsgs.get(i).getNBTValue());

                if (listMsgs.get(i).key.equals("weather.raining")) {

                    NBTTagCompound nbt = listMsgs.get(i)
                        .getNBTValue();
        ...
        if (debugIMC) {
            try {
                ...
                for (int i = 0; i < listMsgs.size(); i++) {
                    ...
                    // System.out.println(listMsgs.get(i).key + " - modID: " + listMsgs.get(i).getSender() + " - source:
                    // " + listMsgs.get(i).toString() + " - " + listMsgs.get(i).getNBTValue());
                }
            } catch (Exception ex) {
                ex.printStackTrace();
            }
