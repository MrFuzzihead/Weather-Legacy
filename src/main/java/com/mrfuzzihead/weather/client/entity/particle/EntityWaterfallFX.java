...
public class EntityWaterfallFX extends EntityRotFX implements WindHandler {
    ...
    public void renderParticle(Tessellator var1, float var2, float var3, float var4, float var5, float var6,
        ...
        var16 = (1F + FMLClientHandler.instance()
            .getClient().gameSettings.gammaSetting) - (this.worldObj.calculateSkylightSubtracted(var2) * 0.13F);
    ...
    public void onUpdate() {
        ...
        if (id.getMaterial() == Material.water/* id == 9 || id == 8 */) {
            ...
            Double dir = BlockLiquid.getFlowDirection(
                worldObj,
                (int) Math.floor(posX),
                (int) Math.floor(posY),
                (int) Math.floor(posZ),
            ...
            meta = this.worldObj
                .getBlockMetadata((int) Math.floor(posX), (int) Math.floor(posY), (int) Math.floor(posZ));
        ...
        if ((id.getMaterial() == Material.water) && motionY > 0F
