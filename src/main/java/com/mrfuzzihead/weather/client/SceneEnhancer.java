package com.mrfuzzihead.weather.client;
...
import com.mrfuzzihead.weather.api.WindReader;
...
import com.mrfuzzihead.weather.client.entity.particle.EntityWaterfallFX;
...
import com.mrfuzzihead.weather.util.WeatherUtilParticle;
import com.mrfuzzihead.weather.weathersystem.WeatherManagerClient;
import com.mrfuzzihead.weather.weathersystem.storm.StormObject;
import com.mrfuzzihead.weather.weathersystem.wind.WindManager;
...
public class SceneEnhancer implements Runnable {
    ...
    public void tickClientThreaded() {
        ...
        if (mc.theWorld != null && mc.thePlayer != null
            && WeatherUtilConfig.listDimensionsWindEffects.contains(mc.theWorld.provider.dimensionId)) {
            profileSurroundings();
            tryAmbientSounds();
        }
    }
                        ...
                        if (block == null
                            ...
                            if (lastPlayTime < System.currentTimeMillis()) {
                                if (cCor.block == SOUNDMARKER_WATER) {
                                    ...
                                    // mc.getSoundHandler().playSound(Weather.modID + ":waterfall", cCor.posX,
                                    // cCor.posY, cCor.posZ, (float)ConfigMisc.volWaterfallScale, 0.75F +
                                    ...
                                    mc.theWorld.playSound(
                                        ...
                                        false);
                                    // System.out.println("play waterfall at: " + cCor.posX + " - " + cCor.posY + " - "
                                    // + cCor.posZ);
                                } else if (cCor.block == SOUNDMARKER_LEAVES) {

                                    float windSpeed = WindReader.getWindSpeed(
                                        mc.theWorld,
                                        Vec3.createVectorHelper(cCor.posX, cCor.posY, cCor.posZ),
                                        WindReader.WindType.EVENT);
                                    if (windSpeed > 0.2F) {
                                        soundTimeLocations
                                            .put(cCor, System.currentTimeMillis() + 12000 + rand.nextInt(50));
                                        // mc.getSoundHandler().playSound(Weather.modID + ":wind_calmfade", cCor.posX,
                                        // cCor.posY, cCor.posZ, (float)(windSpeed * 4F * ConfigMisc.volWindTreesScale),
                                        // 0.70F + (rand.nextFloat() * 0.1F));
                                        mc.theWorld.playSound(
                                            cCor.posX,
                                            cCor.posY,
                                            cCor.posZ,
                                            Weather.modID + ":env.wind_calmfade",
                                            (float) (windSpeed * 4F * ConfigMisc.volWindTreesScale),
                                            0.70F + (rand.nextFloat() * 0.1F),
                                            false);
                                        // System.out.println("play leaves sound at: " + cCor.posX + " - " + cCor.posY +
                                        // " - " + cCor.posZ + " - windSpeed: " + windSpeed);
                                    } else {
                                        ...
                                        if (mc.theWorld.rand.nextInt(15) == 0) {
                                            soundTimeLocations
                                                .put(cCor, System.currentTimeMillis() + 12000 + rand.nextInt(50));
                                            // mc.getSoundHandler().playSound(Weather.modID + ":wind_calmfade",
                                            // cCor.posX, cCor.posY, cCor.posZ, (float)(windSpeed * 2F *
                                            // ConfigMisc.volWindTreesScale), 0.70F + (rand.nextFloat() * 0.1F));
                                            mc.theWorld.playSound(
                                                cCor.posX,
                                                cCor.posY,
                                                cCor.posZ,
                                                Weather.modID + ":env.wind_calmfade",
                                                (float) (windSpeed * 2F * ConfigMisc.volWindTreesScale),
                                                0.70F + (rand.nextFloat() * 0.1F),
                                                false);
                                        }
                                        // System.out.println("play leaves sound at: " + cCor.posX + " - " + cCor.posY +
                                ...
                                // System.out.println("still waiting, diff: " + (lastPlayTime -
                        ...
                        if (block != null) {
                            ...
                            if (ConfigMisc.Wind_Particle_waterfall && ((block.getMaterial() == Material.water))) {
                                ...
                                if ((meta & 8) != 0) {
                                    ...
                                    // this scans to bottom till not water, kinda overkill? owell lets keep it, and also
                                    // add rule if index > 4 (waterfall height of 4)
                                    while (yy - index > 0) {
                                    ...
                                    if (index >= 4 && (block2 != null && block2.getMaterial() == Material.water
                                        && (meta2 & 8) != 0)) {
                                        boolean proxFail = false;
                                        for (int j = 0; j < soundLocations.size(); j++) {
                                            if (Math.sqrt(
                                                soundLocations.get(j)
                                                    .getDistanceSquared(xx, bottomY, zz))
                                                < 5) {
                                                proxFail = true;
                                                break;
                                            }
                                        ...
                                        if (!proxFail) {
                                            soundLocations
                                                .add(new ChunkCoordinatesBlock(xx, bottomY, zz, SOUNDMARKER_WATER, 0));
    ...
    public void tickParticlePrecipitation() {
        ...
        if (ConfigMisc.Particle_RainSnow) {
            EntityPlayer entP = FMLClientHandler.instance()
                .getClient().thePlayer;
            ...
            if (/* true */biomegenbase
                != null/* || biomegenbase.canSpawnLightningBolt() || biomegenbase.getEnableSnow() */) {
                ...
                if (/* curPrecipVal > 0 */entP.worldObj.getWorldChunkManager()
                    ...
                    // rain
                    if (curPrecipVal > 0 && entP.worldObj.canLightningStrikeAt(
                        MathHelper.floor_double(entP.posX),
                        ...
                        for (int i = 0; i < curPrecipVal * 20F * ConfigMisc.Particle_Precipitation_effect_rate; i++) {
                            ...
                            EntityFallingRainFX ent = new EntityFallingRainFX(
                                entP.worldObj,
    ...
    public static float getRainStrengthAndControlVisuals(EntityPlayer entP, boolean forOvercast) {
        ...
        if (forOvercast) {
            // storm = ClientTickHandler.weatherManager.getClosestStorm(plPos, maxStormDist, StormObject.STATE_THUNDER,
            // true);
        } else {
            // storm = ClientTickHandler.weatherManager.getClosestStorm(plPos, maxStormDist, StormObject.STATE_FORMING,
            // true);
        ...
        if (storm != null) {
            ...
            // extend overcast effect, using x2 for now since we cant cancel sound and ground particles, originally was
            // 4x, then 3x, change to that for 1.7 if lex made change
            if (forOvercast) {
                sizeToUse *= 1F;
            }
    ...
    public void profileSurroundings() {
        ...
        float windStr = manager.windMan.getWindSpeedForPriority();// (weatherMan.wind.strength <= 1F ?
                                                                  // weatherMan.wind.strength : 1F);

        if (mc.objectMouseOver != null) {
            Block id = mc.theWorld
        ...
        if ((!ConfigMisc.Wind_Particle_leafs && !ConfigMisc.Wind_Particle_air
            ...
            && !ConfigMisc.Wind_Particle_waterfall)/* || weatherMan.wind.strength < 0.10 */) {
            return;
        }
        ...
        // System.out.println("lastTickFoundBlocks: " + lastTickFoundBlocks + " - rand size: " + uh + " - " +
        // BlockCountRate);
        ...
        for (int xx = curX - hsize; xx < curX + hsize; xx++) {
            for (int yy = curY - (hsize / 2); yy < curY + hsize; yy++) {
                for (int zz = curZ - hsize; zz < curZ + hsize; zz++) {
                    ...
                    if (/* id == ((Block)p_blocks_leaf.get(i)).blockID */block != null
                        && (block.getMaterial() == Material.leaves || block.getMaterial() == Material.vine)) {

                        lastTickFoundBlocks++;

                        if (/* true || */worldRef.rand.nextInt(spawnRate) == 0) {
                            // bottom of tree check || air beside vine check
                            if (ConfigMisc.Wind_Particle_leafs
                                && (CoroUtilBlock.isAir(getBlock(worldRef, xx, yy - 1, zz))
                                    || CoroUtilBlock.isAir(getBlock(worldRef, xx - 1, yy, zz)))) {

                                EntityRotFX var31 = new EntityTexBiomeColorFX(
                                    worldRef,
                                    (double) xx,
                                    (double) yy - 0.5,
                                    (double) zz,
                                 ...
                                 * {
                                 * //This is non leaves, as in wildgrass or wahtever is in the p_blocks_leaf list (no
                                 * special rules)
                                 * EntityRotFX var31 = new EntityTexFX(worldRef, (double)xx, (double)yy + 0.5,
                                 * (double)zz, 0D, 0D, 0D, 10D, 0, effLeafID);
                    ...
                    } else if (ConfigMisc.Wind_Particle_waterfall && player.getDistance(xx, yy, zz) < 16
                        && (block != null && block.getMaterial() == Material.water)) {

                            int meta = getBlockMetadata(worldRef, xx, yy, zz);
                            if ((meta & 8) != 0) {
                                lastTickFoundBlocks += 70; // adding more to adjust for the rate 1 waterfall block spits
                                                           // out particles
                                ...
                                // if ((block2 == null || block2.getMaterial() != Material.water) && (block3 != null &&
                                // block3.getMaterial() == Material.water)) {
                                // chance /= 3;

                                // }
                                // System.out.println("woot! " + chance);
                                if ((((block2 == null || block2.getMaterial() != Material.water) || (meta2 & 8) == 0)
                                    && (block3 != null && block3.getMaterial() == Material.water))
                                    ...
                                    // waterP = new EntityBubbleFX(worldRef, (double)xx + 0.5F + ((rand.nextFloat() *
                                    // range) - (range/2)), (double)yy + 0.5F + ((rand.nextFloat() * range) -
                                    // (range/2)), (double)zz + 0.5F + ((rand.nextFloat() * range) - (range/2)), 0D, 0D,
                                    // 0D);
                                    // } else {
                        ...
                        } else if (ConfigMisc.Wind_Particle_fire
                            && (block != null && block == Blocks.fire/* block.getMaterial() == Material.fire */)) {
                                lastTickFoundBlocks++;
                                ...
                                if (worldRef.rand.nextInt(Math.max(1, (spawnRate / 100))) == 0) {
                                    ...
                                    EntityRotFX entityfx = pm.spawnNewParticleIconFX(
                                        ...
                                        xx + rand.nextDouble(),
                                        yy + 0.2D + rand.nextDouble() * 0.2D,
                                        zz + rand.nextDouble(),
                                        (rand.nextDouble() - rand.nextDouble()) * speed,
                                        0.03D,
                                        (rand.nextDouble() - rand.nextDouble()) * speed);// pm.spawnNewParticleWindFX(worldRef,
                                                                                         // ParticleRegistry.smoke, xx +
                                                                                         // rand.nextDouble(), yy + 0.2D
                                                                                         // + rand.nextDouble() * 0.2D,
                                                                                         // zz + rand.nextDouble(),
                                                                                         // (rand.nextDouble() -
                                                                                         // rand.nextDouble()) * speed,
                                                                                         // 0.03D, (rand.nextDouble() -
                                                                                         // rand.nextDouble()) * speed);
                                ...
                                float temp = worldRef.getBiomeGenForCoords(xx, zz)
                                    .getFloatTemperature(xx, yy, zz);

                                // System.out.println(temp);

                                // Snow!
                                /*
                                 * if (false && ConfigMisc.Wind_Particle_snow && player.getDistance(xx, yy, zz) < 20 &&
                                 * (worldRef.rand.nextInt(100) == 0) && yy == ((int)player.posY+8) && temp <= 0.15F) {
                                 * EntityRotFX snow = new EntitySnowFX(worldRef, (double)xx, (double)yy + 0.5,
                                 * (double)zz, 0D, 0D, 0D, 1F);
                                 ...
                                 * spawnQueue.add(snow);
                                 * }
                                 * if (ConfigMisc.Wind_Particle_air && windStr > 0.05 &&
                                 * worldRef.canBlockSeeTheSky(curX, curY, curZ))
                                 * {
                                 * int chance = 200 - (int)(windStr * 100);
                                 * if (chance <= 0) chance = 1;
                                 * if ((worldRef.rand.nextInt(uh + 0) == 0) && worldRef.rand.nextInt(chance) == 0)
                                 * {
                                 * //EntityFX var31 = new EntitySmokeFX(worldRef, (double)xx, (double)yy+0.5,
                                 * (double)zz, 0D, 0D, 0D);
                                 * EntityRotFX var31 = new EntityTexFX(worldRef, (double)xx, (double)yy + 0.5,
                                 * (double)zz, 0D, 0D, 0D, 10D, 0, effWind2ID);
                     ...
                     * c_w_EntityTexFX var31 = new c_w_EntityTexFX(worldRef, (double)xx, (double)yy+0.5, (double)zz, 0D,
                     * 0D, 0D, 10D, 0, effSandID);
                     * //var31 = new EntityWindFX(worldRef, (double)xx, (double)yy+1.2, (double)zz, 0D, 0.0D, 0D, 9.5D,
                     * 1);
    ...
    public static void tryWind(World world) {
        ...
        int dist = 60;
        // if (side == Side.SERVER) {
        // List list = player.worldObj.getEntitiesWithinAABBExcludingEntity(player, player.boundingBox.expand(dist, 80,
        // dist));
        List list = null;
                    ...
                    if (entity1 instanceof EntityFX) {
                        ...
                        if ((world.getHeightValue(
                            ...
                            if ((entity1 instanceof EntityTexFX)
                                && ((EntityTexFX) entity1).getParticleTextureIndex() == WeatherUtilParticle.effLeafID) {
        ...
        if (WeatherUtilParticle.fxLayers != null && windMan.getWindSpeedForPriority() >= 0.10) {
            ...
            for (int layer = 0; layer < 4; layer++) {
                for (int i = 0; i < WeatherUtilParticle.fxLayers[layer].size(); i++) {
                    ...
                    if ((world
                        .getHeightValue(MathHelper.floor_double(entity1.posX), MathHelper.floor_double(entity1.posZ))
                        - 1 < (int) entity1.posY + 1) || (entity1 instanceof EntityTexFX)) {
                        if ((entity1 instanceof EntityFlameFX)) {
                            if (windMan.getWindSpeedForPriority() >= 0.50) WeatherUtilParticle.setParticleAge(
                                (EntityFX) entity1,
                                WeatherUtilParticle.getParticleAge((EntityFX) entity1) + 2);
                        } else if (entity1 instanceof WindHandler) {
                            if (((WindHandler) entity1).getParticleDecayExtra() > 0
            ...
            for (int layer = 0; layer < ExtendedRenderer.rotEffRenderer.layers; layer++) {
                for (int i = 0; i < ExtendedRenderer.rotEffRenderer.fxLayers[layer].size(); i++) {
                     ...
                     * if ((worldRef.getHeightValue((int)(entity1.posX+0.5F), (int)(entity1.posZ+0.5F))-1 <
                     * (int)entity1.posY+1) || (entity1 instanceof EntityTexFX)) {
                     * if ((entity1 instanceof EntityFlameFX)) {
                     ...
                     * } else if ((entity1 instanceof EntityTexFX) && ((EntityTexFX)entity1).getParticleTextureIndex()
                     * == mod_EntMover.effLeafID) {
                     * if (entity1.motionX < 0.01F && entity1.motionZ < 0.01F) {
        ...
        // First, use volume to represent intensity, maybe get different sound samples for higher level winds as they
        // sound different
        // Second, when facing towards wind, you're ears hear it tearing by you more, when turned 90 degrees you do not,
        // simulate this
    ...
    public static void applyWindForce(Entity ent, double multiplier) {
        ...
        ent.motionX += speed * (double) (-MathHelper.sin(windAngle / 180.0F * (float) Math.PI)
            * MathHelper.cos(0F/* weatherMan.wind.yDirection */ / 180.0F * (float) Math.PI));
        ent.motionZ += speed * (double) (MathHelper.cos(windAngle / 180.0F * (float) Math.PI)
            * MathHelper.cos(0F/* weatherMan.wind.yDirection */ / 180.0F * (float) Math.PI));
        // commented out for weather2, yStrength was 0
        // ent.motionY += weatherMan.wind.yStrength * 0.1D * (double)(-MathHelper.sin((weatherMan.wind.yDirection) /
