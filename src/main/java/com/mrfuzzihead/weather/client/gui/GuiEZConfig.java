package com.mrfuzzihead.weather.client.gui;
...
import com.mrfuzzihead.weather.client.gui.elements.GuiButtonBoolean;
import com.mrfuzzihead.weather.client.gui.elements.GuiButtonCycle;
...
public class GuiEZConfig extends GuiScreen {
    ...
    // slightly different from ZC subgui with its main gui, this one we shouldnt need to show the main gui, just auto
    // select first subgui, Performance
    // treat the subguis as tabs, clicking one auto saves data from that opened gui and switches to the tab they
    // clicked, save & close always visible and always closes gui

    // given that all buttons dont exist at same time, we have to build nbtSendCache as pieces of it come in, and sync
    ...
    // public HashMap<Integer, String> buttonsLookupInt = new HashMap<Integer, String>(); //shouldnt need, just use
    ...
    public void onGuiClosed() {
        ...
        // a fix for container using gui opening on client side that doesnt need slot manip - might not have been
        // needed, below was doing initGui on main gui close
    ...
    public void drawBackground(int par1) {
        ...
        this.drawString(
            ...
            "Weather2 EZ GUI Configuration" + (guiCur.equals("main") ? "" : " - GUI Tab: " + guiCur),
            xStart + 7,
            yStart - 9,
            16777215);
        ...
        if (guiCur.equals(GUI_SUBGUI_PERFORMANCE)) {
            ...
            // this.drawString(this.fontRenderer, "Not done yet! Just use /config and find the Dimension listings, ",
            // xStart+7, yStart2+8, 16777215);
            // this.drawString(this.fontRenderer, "then add the dimension ID", xStart+7, yStart2+8+yEleSize*1,
            // 16777215);
            ...
            try {
                for (int i = 0; i < curDimListCountPerPage; i++) {
                    if (curDimListPage * curDimListCountPerPage + i < listDimIDs.size()) {
                        this.drawString(
                            this.fontRendererObj,
                            "\u00A7" + '6' + listDimNames.get(curDimListPage * curDimListCountPerPage + i),
                            xStart + 7,
                            yStart2 + 8 + yEleSize2 * i,
                            16777215);
                        this.drawString(
                            this.fontRendererObj,
                            "Weather: ",
                            xStart + 7,
                            yStart2 + 28 + yEleSize2 * i,
                            16777215);
                        this.drawString(
                            this.fontRendererObj,
                            "Clouds: ",
                ...
                this.drawString(
                    this.fontRendererObj,
                    "" + (curDimListPage + 1) + "/" + (listDimNames.size() / curDimListCountPerPage + 1),
                    xStart + 80,
                    yStart2 + 194,
                    16777215);
            } catch (Exception ex) {
    ...
    public void updateGuiElements() {
        ...
        while (it.hasNext()) {
            ...
            listSettingWeather.add(tag.getBoolean("com/mrfuzzihead"));
        ...
        if (guiCur.equals(GUI_SUBGUI_PERFORMANCE)) {
            if (WeatherUtilConfig.nbtClientData.hasKey("btn_" + WeatherUtilConfig.CMD_BTN_PERF_STORM))
                ((GuiButtonCycle) buttonsLookup.get(WeatherUtilConfig.CMD_BTN_PERF_STORM)).setIndex(
            ...
            if (WeatherUtilConfig.nbtClientData.hasKey("btn_" + WeatherUtilConfig.CMD_BTN_PERF_NATURE))
                ((GuiButtonCycle) buttonsLookup.get(WeatherUtilConfig.CMD_BTN_PERF_NATURE)).setIndex(
            ...
            if (WeatherUtilConfig.nbtClientData.hasKey("btn_" + WeatherUtilConfig.CMD_BTN_PERF_PRECIPRATE))
                ((GuiButtonCycle) buttonsLookup.get(WeatherUtilConfig.CMD_BTN_PERF_PRECIPRATE)).setIndex(
            ...
            // Weather.dbg("test val check: " + serverDataCache.getInteger("btn_" +
            // WeatherUtilConfig.CMD_BTN_COMP_STORM));
            if (serverDataCache.hasKey("btn_" + WeatherUtilConfig.CMD_BTN_COMP_STORM))
                ((GuiButtonCycle) buttonsLookup.get(WeatherUtilConfig.CMD_BTN_COMP_STORM))
            ...
            if (serverDataCache.hasKey("btn_" + WeatherUtilConfig.CMD_BTN_COMP_LOCK))
                ((GuiButtonCycle) buttonsLookup.get(WeatherUtilConfig.CMD_BTN_COMP_LOCK))
            ...
            if (WeatherUtilConfig.nbtClientData.hasKey("btn_" + WeatherUtilConfig.CMD_BTN_COMP_PARTICLEPRECIP))
                ((GuiButtonCycle) buttonsLookup.get(WeatherUtilConfig.CMD_BTN_COMP_PARTICLEPRECIP)).setIndex(
                    WeatherUtilConfig.nbtClientData.getInteger("btn_" + WeatherUtilConfig.CMD_BTN_COMP_PARTICLEPRECIP));
            if (serverDataCache.hasKey("btn_" + WeatherUtilConfig.CMD_BTN_COMP_SNOWFALLBLOCKS))
                ((GuiButtonCycle) buttonsLookup.get(WeatherUtilConfig.CMD_BTN_COMP_SNOWFALLBLOCKS))
            ...
            if (WeatherUtilConfig.nbtClientData.hasKey("btn_" + WeatherUtilConfig.CMD_BTN_COMP_PARTICLESNOMODS))
                ((GuiButtonCycle) buttonsLookup.get(WeatherUtilConfig.CMD_BTN_COMP_PARTICLESNOMODS)).setIndex(
                    WeatherUtilConfig.nbtClientData
                        .getInteger("btn_" + WeatherUtilConfig.CMD_BTN_COMP_PARTICLESNOMODS));

            ((GuiButtonCycle) buttonsLookup
                .get(WeatherUtilConfig.CMD_BTN_COMP_STORM)).enabled = canPlayerChangeServerSettings;
            ((GuiButtonCycle) buttonsLookup
                .get(WeatherUtilConfig.CMD_BTN_COMP_LOCK)).enabled = canPlayerChangeServerSettings;
            ((GuiButtonCycle) buttonsLookup
                .get(WeatherUtilConfig.CMD_BTN_COMP_SNOWFALLBLOCKS)).enabled = canPlayerChangeServerSettings;

        } else if (guiCur.equals(GUI_SUBGUI_PREFERENCE)) {
            if (serverDataCache.hasKey("btn_" + WeatherUtilConfig.CMD_BTN_PREF_RATEOFSTORM))
                ((GuiButtonCycle) buttonsLookup.get(WeatherUtilConfig.CMD_BTN_PREF_RATEOFSTORM))
            ...
            if (serverDataCache.hasKey("btn_" + WeatherUtilConfig.CMD_BTN_PREF_CHANCEOFSTORM))
                ((GuiButtonCycle) buttonsLookup.get(WeatherUtilConfig.CMD_BTN_PREF_CHANCEOFSTORM))
            ...
            if (serverDataCache.hasKey("btn_" + WeatherUtilConfig.CMD_BTN_PREF_CHANCEOFRAIN))
                ((GuiButtonCycle) buttonsLookup.get(WeatherUtilConfig.CMD_BTN_PREF_CHANCEOFRAIN))
            ...
            if (serverDataCache.hasKey("btn_" + WeatherUtilConfig.CMD_BTN_PREF_BLOCKDESTRUCTION))
                ((GuiButtonCycle) buttonsLookup.get(WeatherUtilConfig.CMD_BTN_PREF_BLOCKDESTRUCTION))
            ...
            if (serverDataCache.hasKey("btn_" + WeatherUtilConfig.CMD_BTN_PREF_TORNADOANDCYCLONES))
                ((GuiButtonCycle) buttonsLookup.get(WeatherUtilConfig.CMD_BTN_PREF_TORNADOANDCYCLONES))
                    .setIndex(serverDataCache.getInteger("btn_" + WeatherUtilConfig.CMD_BTN_PREF_TORNADOANDCYCLONES));

            ((GuiButtonCycle) buttonsLookup
                .get(WeatherUtilConfig.CMD_BTN_PREF_RATEOFSTORM)).enabled = canPlayerChangeServerSettings;
            ((GuiButtonCycle) buttonsLookup
                .get(WeatherUtilConfig.CMD_BTN_PREF_CHANCEOFSTORM)).enabled = canPlayerChangeServerSettings;
            ((GuiButtonCycle) buttonsLookup
                .get(WeatherUtilConfig.CMD_BTN_PREF_CHANCEOFRAIN)).enabled = canPlayerChangeServerSettings;
            ((GuiButtonCycle) buttonsLookup
                .get(WeatherUtilConfig.CMD_BTN_PREF_BLOCKDESTRUCTION)).enabled = canPlayerChangeServerSettings;
            ((GuiButtonCycle) buttonsLookup
                .get(WeatherUtilConfig.CMD_BTN_PREF_TORNADOANDCYCLONES)).enabled = canPlayerChangeServerSettings;
            ...
            try {
                for (int i = 0; i < curDimListCountPerPage; i++) {
                    ...
                    if (curDimListPage * curDimListCountPerPage + i < listDimIDs.size()) {
                        ...
                        ((GuiButtonCycle) buttonsLookup.get(startIndex + 0))
                            .setIndex(listSettingWeather.get(curDimListPage * curDimListCountPerPage + i) ? 1 : 0);
                        ((GuiButtonCycle) buttonsLookup.get(startIndex + 1))
                            .setIndex(listSettingClouds.get(curDimListPage * curDimListCountPerPage + i) ? 1 : 0);
                        ((GuiButtonCycle) buttonsLookup.get(startIndex + 2))
                            .setIndex(listSettingStorms.get(curDimListPage * curDimListCountPerPage + i) ? 1 : 0);

                        // shouldnt this be client side?
                        ((GuiButtonCycle) buttonsLookup.get(startIndex + 3))
                            .setIndex(listSettingEffects.get(curDimListPage * curDimListCountPerPage + i) ? 1 : 0);
                    }
         ...
         * //custom lighting
         * ((GuiButtonBoolean)buttonsLookup.get(LevelConfig.nbtStrCustomLightingUse)).setBoolean(LevelConfig.
         * nbtInfoClientMapConfig.getBoolean(LevelConfig.nbtStrCustomLightingUse));
         * textBoxesLookup.get(LevelConfig.nbtStrCustomLightingMode).setText(LevelConfig.nbtInfoClientMapConfig.
         * getString(LevelConfig.nbtStrCustomLightingMode));
         * //custom time
         * ((GuiButtonBoolean)buttonsLookup.get(LevelConfig.nbtStrCustomTimeUse)).setBoolean(LevelConfig.
         * nbtInfoClientMapConfig.getBoolean(LevelConfig.nbtStrCustomTimeUse));
         * textBoxesLookup.get(LevelConfig.nbtStrCustomTimeVal).setText(LevelConfig.nbtInfoClientMapConfig.getString(
         * LevelConfig.nbtStrCustomTimeVal));
         * } else if (guiCur.equals(GUI_SUBGUI_WAVE)) {
         * textBoxesLookup.get(LevelConfig.nbtStrWaveDefaultMobSpawned).setText(LevelConfig.nbtInfoClientMapConfig.
         * getString(LevelConfig.nbtStrWaveDefaultMobSpawned));
         * textBoxesLookup.get(LevelConfig.nbtStrWaveSpawnCountBase).setText(LevelConfig.nbtInfoClientMapConfig.
         * getString(LevelConfig.nbtStrWaveSpawnCountBase));
         * textBoxesLookup.get(LevelConfig.nbtStrWaveSpawnCountMultiplier).setText(LevelConfig.nbtInfoClientMapConfig.
         * getString(LevelConfig.nbtStrWaveSpawnCountMultiplier));
         * textBoxesLookup.get(LevelConfig.nbtStrWaveHealth).setText(LevelConfig.nbtInfoClientMapConfig.getString(
         * LevelConfig.nbtStrWaveHealth));
         * textBoxesLookup.get(LevelConfig.nbtStrWaveHealthAmp).setText(LevelConfig.nbtInfoClientMapConfig.getString(
         * LevelConfig.nbtStrWaveHealthAmp));
         * textBoxesLookup.get(LevelConfig.nbtStrWaveSpeedBase).setText(LevelConfig.nbtInfoClientMapConfig.getString(
         * LevelConfig.nbtStrWaveSpeedBase));
         * textBoxesLookup.get(LevelConfig.nbtStrWaveSpeedRand).setText(LevelConfig.nbtInfoClientMapConfig.getString(
         * LevelConfig.nbtStrWaveSpeedRand));
         * textBoxesLookup.get(LevelConfig.nbtStrWaveSpeedAmp).setText(LevelConfig.nbtInfoClientMapConfig.getString(
         * LevelConfig.nbtStrWaveSpeedAmp));
         * textBoxesLookup.get(LevelConfig.nbtStrWaveSpeedAmpMax).setText(LevelConfig.nbtInfoClientMapConfig.getString(
         * LevelConfig.nbtStrWaveSpeedAmpMax));
         * textBoxesLookup.get(LevelConfig.nbtStrWaveMoveLeadDist).setText(LevelConfig.nbtInfoClientMapConfig.getString(
         * LevelConfig.nbtStrWaveMoveLeadDist));
         * }
         */
    }
    ...
    public void initGui() {
        ...
        addButton(
            new GuiButton(
                CMD_CLOSE,
                xStart + xSize - guiPadding - btnWidth,
                yStart + ySize - guiPadding - btnHeight,
                btnWidth,
                btnHeight,
                "Save & Close"));
        addButton(
            new GuiButton(
                CMD_ADVANCED,
                xStart + xSize - guiPadding - btnWidth - btnWidthAndPadding,
                yStart + ySize - guiPadding - btnHeight,
                btnWidth,
                btnHeight,
                "Advanced"));
        ...
        addButton(
            new GuiButton(
                ...
                btnWidth,
                btnHeight,
                (guiCur.equals(GUI_SUBGUI_PERFORMANCE) ? "\u00A7" + '2' : "") + GUI_SUBGUI_PERFORMANCE));
        addButton(
            new GuiButton(
                ...
                btnWidth,
                btnHeight,
                (guiCur.equals(GUI_SUBGUI_COMPATIBILITY) ? "\u00A7" + '2' : "") + GUI_SUBGUI_COMPATIBILITY));
        addButton(
            new GuiButton(
                ...
                btnWidth,
                btnHeight,
                (guiCur.equals(GUI_SUBGUI_PREFERENCE) ? "\u00A7" + '2' : "") + GUI_SUBGUI_PREFERENCE));
        addButton(
            new GuiButton(
                ...
                btnWidth,
                btnHeight,
                (guiCur.equals(GUI_SUBGUI_DIMENSIONS) ? "\u00A7" + '2' : "") + GUI_SUBGUI_DIMENSIONS));
        ...
        /*
         * if (guiCur.equals("main")) {
         * addButton(GUI_SUBGUI_WAVE, new GuiButton(CMD_SUBGUI_WAVE, xStartPadded, yStartPadded+btnHeightAndPadding*6,
         * btnWidth, btnHeight, "Conf. Wave"));
         * addButton(LevelConfig.nbtStrCustomLightingUse, new GuiButtonBoolean(CMD_BOOL_CUSTOMLIGHTING, xStartPadded,
         * yStartPadded+btnHeight-8, btnWidthBool, btnHeight, "Enabled", "Disabled"));
         * addTextBox(LevelConfig.nbtStrCustomLightingMode, new GuiTextFieldZC(LevelConfig.nbtStrCustomLightingMode,
         * this.fontRenderer, xStartPadded + 55, yStartPadded+btnHeight-8, 220, 20));
         * addButton(LevelConfig.nbtStrCustomTimeUse, new GuiButtonBoolean(CMD_BOOL_CUSTOMTIME, xStartPadded,
         * yStartPadded+btnHeight*3-8, btnWidthBool, btnHeight, "Enabled", "Disabled"));
         * addTextBox(LevelConfig.nbtStrCustomTimeVal, new GuiTextFieldZC(LevelConfig.nbtStrCustomTimeVal,
         * this.fontRenderer, xStartPadded + 55, yStartPadded+btnHeight*3-8, 220, 20));
         * } else if (guiCur.equals(GUI_SUBGUI_WAVE)) {
         * int x2 = 90;
         * addTextBox(LevelConfig.nbtStrWaveDefaultMobSpawned, new
         * GuiTextFieldZC(LevelConfig.nbtStrWaveDefaultMobSpawned, this.fontRenderer, xStartPadded,
         * yStartPadded+btnHeight-8, 220, 20));
         * addTextBox(LevelConfig.nbtStrWaveSpawnCountBase, new GuiTextFieldZC(LevelConfig.nbtStrWaveSpawnCountBase,
         * this.fontRenderer, xStartPadded, yStartPadded+btnHeight*3-8, 60, 20));
         * addTextBox(LevelConfig.nbtStrWaveSpawnCountMultiplier, new
         * GuiTextFieldZC(LevelConfig.nbtStrWaveSpawnCountMultiplier, this.fontRenderer, xStartPadded + x2,
         * yStartPadded+btnHeight*3-8, 60, 20));
         * addTextBox(LevelConfig.nbtStrWaveHealth, new GuiTextFieldZC(LevelConfig.nbtStrWaveHealth, this.fontRenderer,
         * xStartPadded, yStartPadded+btnHeight*5-8, 60, 20));
         * addTextBox(LevelConfig.nbtStrWaveHealthAmp, new GuiTextFieldZC(LevelConfig.nbtStrWaveHealthAmp,
         * this.fontRenderer, xStartPadded + x2, yStartPadded+btnHeight*5-8, 60, 20));
         * addTextBox(LevelConfig.nbtStrWaveSpeedBase, new GuiTextFieldZC(LevelConfig.nbtStrWaveSpeedBase,
         * this.fontRenderer, xStartPadded, yStartPadded+btnHeight*7-8, 60, 20));
         * addTextBox(LevelConfig.nbtStrWaveSpeedRand, new GuiTextFieldZC(LevelConfig.nbtStrWaveSpeedRand,
         * this.fontRenderer, xStartPadded + x2, yStartPadded+btnHeight*7-8, 60, 20));
         * addTextBox(LevelConfig.nbtStrWaveSpeedAmp, new GuiTextFieldZC(LevelConfig.nbtStrWaveSpeedAmp,
         * this.fontRenderer, xStartPadded + x2*2, yStartPadded+btnHeight*7-8, 60, 20));
         * addTextBox(LevelConfig.nbtStrWaveSpeedAmpMax, new GuiTextFieldZC(LevelConfig.nbtStrWaveSpeedAmpMax,
         * this.fontRenderer, xStartPadded + x2*3, yStartPadded+btnHeight*7-8, 60, 20));
         * addTextBox(LevelConfig.nbtStrWaveMoveLeadDist, new GuiTextFieldZC(LevelConfig.nbtStrWaveMoveLeadDist,
         * this.fontRenderer, xStartPadded, yStartPadded+btnHeight*9-8, 60, 20));
         * }
         */
    ...
    protected void actionPerformed(GuiButton var1) {
        ...
        if (WeatherUtilConfig.listSettingsServer.contains(var1.id)) {
            if (var1 instanceof GuiButtonBoolean) {
                ...
                nbtSendCache.getCompoundTag("guiData")
            ...
            || var1.id == CMD_SUBGUI_DIMENSIONS) {
                 ...
                 * mc.thePlayer.closeScreen();
                 * } else if (guiCur.equals(GUI_SUBGUI_PERFORMANCE) || guiCur.equals(GUI_SUBGUI_COMPATIBILITY) ||
                 * guiCur.equals(GUI_SUBGUI_PREFERENCE)) {
                 * guiCur = guiPrev;
                 * initGui();
                 * }
                 */
                ...
                try {
                    ...
                    if ((var1.id - btnDimIndexStart) % 4 == 3) {
                        ...
                        nbtDims.setInteger(
                            "dim_"
                                + listDimIDs.get(
                                    (curDimListPage * curDimListCountPerPage)
                                        + (var1.id - btnDimIndexStart) / curDimListCountPerPage)
                                + "_"
                                + ((var1.id - btnDimIndexStart) % 4),
                            ...
                            ((var1.id - btnDimIndexStart) % 4) + " - "
                                + listDimIDs.get(
                                    (curDimListPage * curDimListCountPerPage)
                                        + (var1.id - btnDimIndexStart) / curDimListCountPerPage));
                        ((GuiButtonCycle) var1).cycleIndex();
                        NBTTagCompound nbtDims = nbtSendCache.getCompoundTag("guiData")
