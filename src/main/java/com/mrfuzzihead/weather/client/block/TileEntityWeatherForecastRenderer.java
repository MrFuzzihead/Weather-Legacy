package com.mrfuzzihead.weather.client.block;
...
import com.mrfuzzihead.weather.ClientProxy;
...
public class TileEntityWeatherForecastRenderer extends TileEntitySpecialRenderer {
    ...
    public void renderTileEntityAt(TileEntity var1, double x, double y, double z, float var8) {
        ...
        String descSeverity = "";
        String descDist = "";
        String descWindAngleCloud = "Wind Angle Clouds: "
            + (int) WindReader.getWindAngle(var1.getWorldObj(), pos, WindReader.WindType.CLOUD);
        String descWindAngle = "Wind Angle Effect: "
            + (int) WindReader.getWindAngle(var1.getWorldObj(), pos, WindReader.WindType.DOMINANT);
        String descWindSpeed = "Wind Speed Effect: "
            + (((int) (WindReader.getWindSpeed(var1.getWorldObj(), pos, WindReader.WindType.DOMINANT) * 100F)) / 100F);
        ...
        // renderLivingLabel("closest storm type: " + progression + descSeverity, x, y + yOffset - 0.1F * index++, z, 1,
        // RenderManager.instance.playerViewY);
        // renderLivingLabel("closest storm dist: " + descDist, x, y + yOffset - 0.1F * index++, z, 1,
        // RenderManager.instance.playerViewY);
        // if (so != null) renderLivingLabel("closest storm intensity: " + ((int)(so.levelCurStagesIntensity * 100F) /
        // 100F), x, y + yOffset - 0.1F * index++, z, 1, RenderManager.instance.playerViewY);
        // if (so != null) renderLivingLabel("closest storm water level: " + (so.attrib_precipitation ? so.levelWater :
        // 0), x, y + yOffset - 0.1F * index++, z, 1, RenderManager.instance.playerViewY);
        renderLivingLabel(
            descWindAngleCloud,
            x,
            y + yOffset - 0.1F * index++,
            z,
            1,
            RenderManager.instance.playerViewY);
        ...
        for (int i = 0; i < tEnt.storms.size(); i++) {
            ...
            Vec3 posRenderOffset = Vec3
                .createVectorHelper(storm.pos.xCoord - tEnt.xCoord, 0, storm.pos.zCoord - tEnt.zCoord);
            ...
            if (storm.levelCurIntensityStage >= StormObject.STATE_FORMING) {
                if (storm.stormType == StormObject.TYPE_WATER) {
                    renderIcon(
                        ...
                        16,
                        RenderManager.instance.playerViewY,
                        ClientProxy.radarIconCyclone);
                    renderLivingLabel(
                        "C" + (int) (storm.levelCurIntensityStage - StormObject.levelStormIntensityFormingStartVal),
                        x,
                        y + 1.5F,
                        z,
                        1,
    ...
    protected void renderLivingLabel(String par2Str, double par3, double par5, double par7, int par9, int width,
