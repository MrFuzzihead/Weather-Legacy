package com.mrfuzzihead.weather.client.sound;
...
public class MovingSoundStreamingSource extends MovingSound {
    ...
    public void update() {
        ...
        if (storm != null) {
            realSource = Vec3.createVectorHelper(
                this.storm.posGround.xCoord,
                this.storm.posGround.yCoord,
                this.storm.posGround.zCoord);
        }

        float var3 = (float) ((cutOffRange - (double) MathHelper
            .sqrt_double(getDistanceFrom(realSource, Vec3.createVectorHelper(entP.posX, entP.posY, entP.posZ))))
            / cutOffRange);
