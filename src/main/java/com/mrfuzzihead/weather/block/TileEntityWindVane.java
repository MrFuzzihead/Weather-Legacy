    ...
    public void updateEntity() {
        if (worldObj.isRemote) {
            ...
            if (worldObj.getTotalWorldTime() % 40 == 0) {
                isOutsideCached = WeatherUtilEntity
                    .isPosOutside(worldObj, Vec3.createVectorHelper(xCoord + 0.5F, yCoord + 0.5F, zCoord + 0.5F));
            }

            if (isOutsideCached) {
                // x1 * y2 - y1 * x2 cross product to get optimal turn angle, errr i have angle and target angle, not
                // positions...
                ...
                if (Math.abs(bestMove) < 180/* - (angleAdjust * 2) */) {
                    ...
                    // System.out.println("smoothAngle: " + smoothAngle + " - smoothAngleRotationalVel: " +
                    // smoothAngleRotationalVel + " - smoothAngleRotationalVelAccel: " + smoothAngleRotationalVelAccel);
                }
            }
        }
    }
