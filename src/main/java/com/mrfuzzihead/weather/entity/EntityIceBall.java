package com.mrfuzzihead.weather.entity;
...
public class EntityIceBall extends EntityThrowableUsefull implements IWindHandler {
    ...
    public void setThrowableHeading(double x, double y, double z, float speed, float inaccuracy) {
        this.func_70186_c(x, y, z, speed, inaccuracy);
    }
    ...
    protected void onImpact(MovingObjectPosition movingobjectposition) {
        ...
        if (movingobjectposition.entityHit != null) {
            if (!worldObj.isRemote) {
                ...
                if (movingobjectposition.entityHit instanceof ICoroAI && getThrower() instanceof ICoroAI) {
                    if (((ICoroAI) getThrower()).getAIAgent().dipl_info
                 ...
                 * {
                 * if (((EntityKoaMember) thrower).dipl_team != ((EntityKoaMember)
                 * movingobjectposition.entityHit).dipl_team) {
                 * movingobjectposition.entityHit.attackEntityFrom(DamageSource.causeThrownDamage(this, thrower),
                 * byte0);
                 * } else {
                 * }
                 * }
                 * else if (!(movingobjectposition.entityHit instanceof EntityKoaMemberNew)) {
                 * if (!movingobjectposition.entityHit.attackEntityFrom(DamageSource.causeThrownDamage(this, thrower),
                 * byte0));
                 * if (thrower instanceof EntityPlayer) {
                 * int what = 0;
                 * }
                 * } else if (movingobjectposition.entityHit instanceof EntityKoaMemberNew && thrower instanceof
                 * EntityKoaMemberNew) {
                 * if (((EntityKoaMemberNew) thrower).dipl_team != ((EntityKoaMemberNew)
                 * movingobjectposition.entityHit).dipl_team) {
                 * movingobjectposition.entityHit.attackEntityFrom(DamageSource.causeThrownDamage(this, thrower),
                 * byte0);
                 * } else {
                 ...
                 * double speed = 0.01D;
                 * EntityTexFX var31 = new EntityTexFX(worldObj, posX, posY, posZ,
                 * rand.nextGaussian()*rand.nextGaussian()*speed, rand.nextGaussian()*speed,
                 * rand.nextGaussian()*rand.nextGaussian()*speed, (rand.nextInt(80)/10), 0, mod_EntMover.effLeafID);
        ...
        if (!worldObj.isRemote) {
            worldObj.playSoundEffect(posX, posY, posZ, "step.stone", 3F, 5F);// 0.2F + worldObj.rand.nextFloat() *
                                                                             // 0.1F);
            setDead();
