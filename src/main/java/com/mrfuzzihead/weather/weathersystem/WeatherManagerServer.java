...
public class WeatherManagerServer extends WeatherManagerBase {
            ...
            if (shouldUpdateHighWind || shouldUpdateLowWind) {
                ...
                for (int i = 0; i < getStormObjects().size(); i++) {
                    ...
                    if (so.levelCurIntensityStage >= StormObject.STATE_HIGHWIND) {
                        if (shouldUpdateHighWind) stormObjectsData.add(so.nbtSyncForClient());
                        // syncStormUpdate(so);
                    } else if (shouldUpdateLowWind) stormObjectsData.add(so.nbtSyncForClient());
                }
    ...
    public void trySpawnNearPlayerForLayer(EntityPlayer entP, int layer) {
        ...
        while (tryCountCur++ == 0 || (tryCountCur < tryCountMax && (soClose != null || playerClose != null))) {
            spawnX = (int) (entP.posX - vecX
                + rand.nextInt(ConfigMisc.Misc_simBoxRadiusSpawn)
                - rand.nextInt(ConfigMisc.Misc_simBoxRadiusSpawn));
            spawnZ = (int) (entP.posZ - vecZ
                + rand.nextInt(ConfigMisc.Misc_simBoxRadiusSpawn)
                - rand.nextInt(ConfigMisc.Misc_simBoxRadiusSpawn));
    ...
    public void syncStormNew(StormObject parStorm, EntityPlayerMP entP) {
        ...
        // PacketDispatcher.sendPacketToAllAround(parStorm.pos.xCoord, parStorm.pos.yCoord, parStorm.pos.zCoord,
        // syncRange, getWorld().provider.dimensionId,
        // WeatherPacketHelper.createPacketForServerToClientSerialization("WeatherData", data));
    }
