...
import com.mrfuzzihead.weather.volcano.VolcanoObject;
...
public class WeatherManagerBase {
    ...
    public void reset() {
        ...
        try {
            lookupStormObjectsByLayer.get(0)
                .clear();
            lookupStormObjectsByLayer.get(1)
                .clear();
            lookupStormObjectsByLayer.get(2)
                .clear();
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        ...
        if (world != null) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (this instanceof WeatherManagerServer && so.isDead) {
                     ...
                     * Weather.dbg("WARNING!!! - detected no packets received in last 60 seconds for storm ID: " + so.ID
                     * + " this is an ongoing bug, force removing storm on client side");
                     * removeStormObject(so.ID);
                     * //if it failed still
                     * if (!so.isDead) {
    ...
    public void addStormObject(StormObject so) {
        if (!lookupStormObjectsByID.containsKey(so.ID)) {
            ...
            lookupStormObjectsByLayer.get(so.layer)
                .add(so);
        } else {
            Weather.dbg(
    ...
    public void writeToFile() {
        ...
        String saveFolder = WeatherFileUtil.getWorldSavePath() + "com/mrfuzzihead" + File.separator;
    ...
    public void readFromFile() {
        ...
        if (!readFail) {
            try {
                ...
                if ((new File(saveFolder + "WeatherData_" + dim + ".dat").exists())) FileUtils.copyFile(
                    (new File(saveFolder + "WeatherData_" + dim + ".dat")),
                    (new File(saveFolder + "WeatherData_" + dim + "_BACKUP0.dat")));
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        ...
        Iterator it = nbtVolcanoes.func_150296_c()
            .iterator();
