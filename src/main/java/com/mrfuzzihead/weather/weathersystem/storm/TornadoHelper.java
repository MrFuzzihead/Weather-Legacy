...
public class TornadoHelper {
    ...
    public void tick(World parWorld) {
        ...
        if (!parWorld.isRemote && ConfigMisc.Storm_Tornado_grabBlocks/* getStorm().grabsBlocks */) {
            ...
            BiomeGenBase bgb = parWorld.getBiomeGenForCoords(
                MathHelper.floor_double(storm.pos.xCoord),
                MathHelper.floor_double(storm.pos.zCoord));
            ...
            if (bgb != null && bgb.rootHeight + bgb.heightVariation <= 0.7) {
                ...
                for (int i = yStart; i < yEnd; i += yInc) {
                    ...
                    int extraTry = (int) ((storm.levelCurIntensityStage + 1 - storm.levelStormIntensityFormingStartVal)
                        * 5);
                    ...
                    for (int k = 0; k < loopAmount; k++) {
                        ...
                        int tryX = (int) storm.pos.xCoord + rand.nextInt(tornadoBaseSize + (ii))
                            - ((tornadoBaseSize / 2) + (ii / 2));
                        int tryZ = (int) storm.pos.zCoord + rand.nextInt(tornadoBaseSize + (ii))
                            - ((tornadoBaseSize / 2) + (ii / 2));
                        ...
                        if (dist < tornadoBaseSize / 2 + ii / 2 && tryRipCount < tryRipMax) {
                            ...
                            if (!CoroUtilBlock.isAir(blockID)
                                && canGrab(parWorld, blockID)/*
                                                              * && Block.blocksList[blockID].blockMaterial ==
                                                              * Material.ground
                                                              *//*
                                                                 * && worldObj.getHeightValue(tryX, tryZ)-1 ==
                                                                 * tryY
                                                                 */) {
                                /*
                        ...
                        // }
                        // int tryX =
                        // (int)posX+this.rand.nextInt(mod_EntMover.tornadoBaseSize+(ii))-((mod_EntMover.tornadoBaseSize/2)+(ii/2));
                        // int tryZ =
                        // (int)posZ+this.rand.nextInt(mod_EntMover.tornadoBaseSize+(ii))-((mod_EntMover.tornadoBaseSize/2)+(ii/2));
                    }
                }
    ...
    public boolean tryRip(World parWorld, int tryX, int tryY, int tryZ, boolean notify) {
        ...
        ((parWorld.getHeightValue(tryX, tryZ) - 1 == tryY) || parWorld.getHeightValue(tryX + 1, tryZ) - 1 < tryY
         ...
         */
            ...
            if (parWorld.getChunkProvider()
                ...
                && tickGrabCount < ConfigMisc.Storm_Tornado_maxBlocksGrabbedPerTick) {
        ...
        if (list != null) {
            for (int i = 0; i < list.size(); i++) {
                ...
                if ((!(entity1 instanceof EntityPlayer) || ConfigMisc.Storm_Tornado_grabPlayer)) {
                    ...
                    if (getDistanceXZ(storm.posBaseFormationPos, entity1.posX, entity1.posY, entity1.posZ) < dist) {
                        if ((entity1 instanceof EntityMovingBlock
                            && !((EntityMovingBlock) entity1).collideFalling)/* || canEntityBeSeen(entity, entity1) */) {
                            storm.spinEntity(entity1);
                            // spin(entity, conf, entity1);
                            foundEnt = true;
                        } else {
                            if (entity1 instanceof EntityPlayer) {
                                // dont waste cpu on server side doing LOS checks, since player movement is client side
                                // only, in all situations ive seen
                                ...
                                && WeatherUtilEntity.isEntityOutside(entity1, true)) {// OldUtil.canVecSeeCoords(parWorld,
                                                                                      // storm.pos, entity1.posX,
                                                                                      // entity1.posY,
                                                                                      // entity1.posZ)/*OldUtil.canEntSeeCoords(entity1,
                                                                                      // entity.posX, entity.posY + 80,
                                                                                      // entity.posZ)*/) {
                                    // trying only server side to fix warp back issue (which might mean client and
                                    // server are mismatching for some rules)
                                    // if (!entity1.worldObj.isRemote) {
                                    storm.spinEntity(entity1);
                                    // spin(entity, conf, entity1);
                                    foundEnt = true;
                ...
                if (entity1 instanceof EntityMovingBlock && !entity1.isDead) {
                    ...
                    byte var5 = 32;
                    /*
                     * if(!entity1.worldObj.checkChunksExist(var3 - var5, 0, var4 - var5, var3 + var5, 128, var4 + var5)
                     * || !entity1.addedToChunk) {
                ...
                if (entity1 instanceof EntityMovingBlock) {
                    ...
                    /*
                     * if (entity1.motionX < 0.3F && entity1.motionY < 0.3F && entity1.motionZ < 0.3F && getFPS() < 20
                     * && killCount < 20)
                     * {
                     * killCount++;
                     * entity1.setDead();
    ...
    public void soundUpdates(boolean playFarSound, boolean playNearSound) {
        ...
        if (distToPlayer < far) {
            ...
            if (storm.levelCurIntensityStage >= storm.STATE_FORMING
                && storm.stormType == storm.TYPE_LAND/* getStorm().type == getStorm().TYPE_TORNADO */) {
         ...
         * {
         * WeatherUtilSound.setVolume(new StringBuilder().append("sound_" + WeatherUtilSound.soundID[0]).toString(),
         * volScaleClose);
         * }
         * if (WeatherUtilSound.soundID[1] > -1 && WeatherUtilSound.soundTimer[1] < System.currentTimeMillis())
         * {
         * WeatherUtilSound.setVolume(new StringBuilder().append("sound_" + WeatherUtilSound.soundID[1]).toString(),
         * volScaleClose);
         * }
         * if (WeatherUtilSound.soundID[2] > -1 && WeatherUtilSound.soundTimer[2] < System.currentTimeMillis())
         * {
         * WeatherUtilSound.setVolume(new StringBuilder().append("sound_" + WeatherUtilSound.soundID[2]).toString(),
         * volScaleFar);
         * }
         * }
         */
        ...
        // worldObj.playRecord("tornado.destruction", (int)mc.thePlayer.posX, (int)mc.thePlayer.posY,
    ...
    public boolean tryPlaySound(String[] sound, int arrIndex, Entity source, float vol, float parCutOffRange) {
        ...
        if (WeatherUtilSound.soundTimer[arrIndex] <= System.currentTimeMillis()) {
            // worldObj.playSoundAtEntity(soundTarget, new StringBuilder().append("tornado."+sound).toString(), 1.0F,
            // 1.0F);
            // ((IWorldAccess)this.worldAccesses.get(var5)).playSound(var2, var1.posX, var1.posY - (double)var1.yOffset,
            // var1.posZ, var3, var4);
            /* WeatherUtilSound.soundID[arrIndex] = */WeatherUtilSound.playMovingSound(
                storm,
                new StringBuilder().append(Weather.modID + ":streaming." + sound[WeatherUtilSound.snd_rand[arrIndex]])
                    .toString(),
                vol,
                1.0F,
                parCutOffRange);
