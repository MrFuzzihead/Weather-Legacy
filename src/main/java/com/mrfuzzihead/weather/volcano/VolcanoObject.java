package com.mrfuzzihead.weather.volcano;
...
public class VolcanoObject {
    ...
    // state 4
    public int ticksToCooldown = 20 * 30;// 20*60*5; //this should ideally be larger than maxSize so it can full
                                         // cooldown (does 1 y layer per 1 processRateDelay)
    ...
    public void tick() {
        ...
        if (side == Side.CLIENT) {
            ...
            if (state == 0) {
                ...
                topBlockID = world.getBlock(
                    MathHelper.floor_double(pos.xCoord),
                    MathHelper.floor_double(pos.yCoord - 1),
                    MathHelper.floor_double(pos.zCoord));

                if (CoroUtilBlock.isAir(topBlockID)
                    || !topBlockID.isBlockSolid(world, (int) pos.xCoord, (int) pos.yCoord - 1, (int) pos.zCoord, 0)) {
                ...
                if (this.manager.getWorld()
                    .getTotalWorldTime() % processRateDelay == 0) {
                    // if (isGrowing) {
                    size++;
                    curHeight++;
                    curRadius++;
                    ...
                    for (int yy = 0; yy <= curHeight; yy++) {
                        ...
                        for (double angle = 0; angle <= 360; angle += res) {
                            ...
                            if (rand.nextInt(4) == 0) {
                                ...
                                while ((CoroUtilBlock.isAir(underBlockID)
                                    || underBlockID.getMaterial() == Material.water) && underY > 1) {
    ...
    public void tickClient() {
        ...
        // if (state == 3 || state == 4) {
        if (this.manager.getWorld()
            .getTotalWorldTime() % delay == 0) {
            for (int i = 0; i < loopSize; i++) {
                if (listParticlesSmoke.size() < 500) {
                    double spawnRad = size / 48;
                    EntityRotFX particle = spawnSmokeParticle(
    ...
    public EntityRotFX spawnSmokeParticle(double x, double y, double z) {
        double speed = 0D;
        Random rand = new Random();
        EntityRotFX entityfx = particleBehaviors.spawnNewParticleIconFX(
