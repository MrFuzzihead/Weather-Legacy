...
public class WeatherUtilConfig {
    ...
    public static List<String> LIST_CHANCE = new ArrayList<String>(
        Arrays.asList(
            "1/2 Day",
            "1 Day",
            "2 Days",
    ...
    public static List<String> LIST_STORMSWHEN = new ArrayList<String>(
        Arrays.asList("Local Biomes", "Global Overcast"));
    ...
    public static void processNBTToModConfigClient() {
        ...
        try {
            if (nbtClientData.hasKey("btn_" + CMD_BTN_COMP_PARTICLEPRECIP)) {
                ConfigMisc.Particle_RainSnow = LIST_TOGGLE
                    .get(nbtClientData.getInteger("btn_" + CMD_BTN_COMP_PARTICLEPRECIP))
                    .equalsIgnoreCase("on");
            }

            if (nbtClientData.hasKey("btn_" + CMD_BTN_PERF_STORM)) {
                if (LIST_RATES.get(nbtClientData.getInteger("btn_" + CMD_BTN_PERF_STORM))
                    .equalsIgnoreCase("high")) {
                    ConfigMisc.Cloud_ParticleSpawnDelay = 0;
                } else if (LIST_RATES.get(nbtClientData.getInteger("btn_" + CMD_BTN_PERF_STORM))
            ...
            if (nbtClientData.hasKey("btn_" + CMD_BTN_PERF_NATURE)) {
                if (LIST_RATES2.get(nbtClientData.getInteger("btn_" + CMD_BTN_PERF_NATURE))
                    .equalsIgnoreCase("high")) {
                    ConfigMisc.Wind_Particle_effect_rate = 1F;
                } else if (LIST_RATES2.get(nbtClientData.getInteger("btn_" + CMD_BTN_PERF_NATURE))
                    .equalsIgnoreCase("medium")) {
            ...
            if (nbtClientData.hasKey("btn_" + CMD_BTN_PERF_PRECIPRATE)) {
                ...
                if (LIST_RATES2.get(nbtClientData.getInteger("btn_" + CMD_BTN_PERF_PRECIPRATE))
                    .equalsIgnoreCase("high")) {
                    ConfigMisc.Particle_Precipitation_effect_rate = 1D;
                } else if (LIST_RATES2.get(nbtClientData.getInteger("btn_" + CMD_BTN_PERF_PRECIPRATE))
                    .equalsIgnoreCase("medium")) {
            ...
            if (nbtClientData.hasKey("btn_" + CMD_BTN_COMP_PARTICLESNOMODS)) {
                ConfigMisc.Particle_VanillaAndWeatherOnly = LIST_TOGGLE
                    .get(nbtClientData.getInteger("btn_" + CMD_BTN_COMP_PARTICLESNOMODS))
                    .equalsIgnoreCase("on");
            }
            ...
            Iterator it = nbtDims.func_150296_c()
                .iterator();
            while (it.hasNext()) {
        ...
        ConfigMod.configLookup.get(modID)
    ...
    public static void processNBTToModConfigServer() {
        ...
        try {
            if (nbtServerData.hasKey("btn_" + CMD_BTN_COMP_STORM)) {
                ConfigMisc.overcastMode = LIST_STORMSWHEN.get(nbtServerData.getInteger("btn_" + CMD_BTN_COMP_STORM))
                    .equalsIgnoreCase("Global Overcast");
            }

            if (nbtServerData.hasKey("btn_" + CMD_BTN_COMP_LOCK)) {
                if (LIST_LOCK.get(nbtServerData.getInteger("btn_" + CMD_BTN_COMP_LOCK))
                    .equalsIgnoreCase("on")) {
                    ConfigMisc.lockServerWeatherMode = 1;
                } else if (LIST_LOCK.get(nbtServerData.getInteger("btn_" + CMD_BTN_COMP_LOCK))
            ...
            if (nbtServerData.hasKey("btn_" + CMD_BTN_COMP_SNOWFALLBLOCKS)) {
                boolean val = LIST_TOGGLE.get(nbtServerData.getInteger("btn_" + CMD_BTN_COMP_SNOWFALLBLOCKS))
                    .equalsIgnoreCase("on");
            ...
            if (nbtServerData.hasKey("btn_" + CMD_BTN_PREF_RATEOFSTORM)) {
                ...
                if (numDays == 0) {
                    ...
                    // potentially remove the 'never' clause from here in favor of the dimension specific disabling of
                    // 'storms' which is already used in code
                    // for now consider this a second layer of rules to the storm creation process, probably not a user
                    // friendly idea
            ...
            if (nbtServerData.hasKey("btn_" + CMD_BTN_PREF_CHANCEOFSTORM)) {
                if (LIST_RATES2.get(nbtServerData.getInteger("btn_" + CMD_BTN_PREF_CHANCEOFSTORM))
                    .equalsIgnoreCase("high")) {
                    ConfigMisc.Player_Storm_Deadly_OddsTo1 = 30;
                    ConfigMisc.Server_Storm_Deadly_OddsTo1 = 30;
                } else if (LIST_RATES2.get(nbtServerData.getInteger("btn_" + CMD_BTN_PREF_CHANCEOFSTORM))
                    .equalsIgnoreCase("medium")) {
            ...
            if (nbtServerData.hasKey("btn_" + CMD_BTN_PREF_CHANCEOFRAIN)) {
                if (LIST_RATES2.get(nbtServerData.getInteger("btn_" + CMD_BTN_PREF_CHANCEOFRAIN))
                    .equalsIgnoreCase("high")) {
                    ConfigMisc.Player_Storm_Rain_OddsTo1 = 150;
                } else if (LIST_RATES2.get(nbtServerData.getInteger("btn_" + CMD_BTN_PREF_CHANCEOFRAIN))
                    .equalsIgnoreCase("medium")) {
            ...
            if (nbtServerData.hasKey("btn_" + CMD_BTN_PREF_BLOCKDESTRUCTION)) {
                ConfigMisc.Storm_Tornado_grabBlocks = LIST_TOGGLE
                    .get(nbtServerData.getInteger("btn_" + CMD_BTN_PREF_BLOCKDESTRUCTION))
                    .equalsIgnoreCase("on");
            }
    ...
    public static NBTTagCompound createNBTDimensionListing() {
        ...
        for (int i = 0; i < worlds.length; i++) {
            ...
            nbtDim.setBoolean("com/mrfuzzihead", listDimensionsWeather.contains(dimID));
            nbtDim.setBoolean("clouds", listDimensionsClouds.contains(dimID));
            nbtDim.setBoolean("storms", listDimensionsStorms.contains(dimID));

            // PROCESS ME ELSEWHERE!!! - must be done in EZGUI post receiving of this data because client still needs
            // this server created dimension listing first
    ...
    public static void nbtWriteNBTToDisk(NBTTagCompound parData, boolean saveForClient) {
        String fileURL = null;
        if (saveForClient) {
            fileURL = WeatherFileUtil.getMinecraftDataPath() + File.separator
                + "Weather2"
                + File.separator
