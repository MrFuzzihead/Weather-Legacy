...
import com.mrfuzzihead.weather.client.sound.MovingSoundStreamingSource;
...
public class WeatherUtilSound {
     ...
     * if (sndSystem != null)
     * {
     * sndSystem.setVolume(new StringBuilder().append(soundID).toString(), vol *
     * FMLClientHandler.instance().getClient().gameSettings.getSoundLevel(SoundCategory.WEATHER));
     * }
     * } catch (Exception ex) {
     ...
     * {
     * SoundManager temp = (SoundManager) OldUtil.getPrivateValueBoth(SoundHandler.class,
     * FMLClientHandler.instance().getClient().getSoundHandler(), "field_147694_f", "sndManager");
    ...
    public static void playMovingSound(StormObject parStorm, String var1, float var5, float var6,
         ...
         * sndSystem.newStreamingSource(true, var1, getURLForSoundResource(res), snd, false, var2, var3, var4,
         * ISound.AttenuationType.NONE.getTypeInt(), 1F);
         * sndSystem.setVolume(snd, var5 *
         * FMLClientHandler.instance().getClient().gameSettings.getSoundLevel(SoundCategory.WEATHER));
         * sndSystem.play(snd);
         ...
         * } catch (Exception ex) {
         * ex.printStackTrace();
         * //despite forcing a getSoundSystem(); call, some are still getting a crash from inside usage of
         * sndSystem.backgroundMusic, this is an attempt to silently catch it
         * //iirc original issue was resource pack switching
         * //second note: issue was still happening, moved try catch to encapsulate entire method, might return invalid
         * sound id now on exception catch
         * //third note: continued issue was where other methods call setVolume, since getSoundSystem is reflectionless,
         * just force it to be called whenever something wants to setVolume or play special sound way
         * }
         */
    }
