...
public class WeatherUtilEntity {
    ...
    public static float getWeight(Entity entity1, boolean forTornado) {
        ...
        if (/* entity1 instanceof EntitySurfboard || */entity1 instanceof EntityBoat || entity1 instanceof EntityItem
            ...
            || entity1 instanceof EntityFishHook) {
            return 4000F;
        }
    ...
    public static boolean canPushEntity(Entity ent) {
        ...
        int startX = (int) (ent.posX
            - speed * (double) (-MathHelper.sin(windMan.getWindAngleForPriority() / 180.0F * (float) Math.PI)
                * MathHelper.cos(0F/* weatherMan.wind.yDirection */ / 180.0F * (float) Math.PI)));
        int startZ = (int) (ent.posZ
            - speed * (double) (MathHelper.cos(windMan.getWindAngleForPriority() / 180.0F * (float) Math.PI)
                * MathHelper.cos(0F/* weatherMan.wind.yDirection */ / 180.0F * (float) Math.PI)));
        ...
        return ent.worldObj.rayTraceBlocks(
            ...
            Vec3.createVectorHelper(startX, ent.posY + (double) ent.getEyeHeight(), startZ)) == null;
        // return true;
    }
    ...
    public static boolean isPosOutside(World parWorld, Vec3 parPos, boolean cheapCheck) {
        ...
        if (parWorld.getHeightValue(MathHelper.floor_double(parPos.xCoord), MathHelper.floor_double(parPos.zCoord))
            < parPos.yCoord + 1) return true;

        if (cheapCheck) return false;

        Vec3 vecTry = Vec3.createVectorHelper(
            parPos.xCoord + ForgeDirection.NORTH.offsetX * rangeCheck,
            parPos.yCoord + yOffset,
            parPos.zCoord + ForgeDirection.NORTH.offsetZ * rangeCheck);
        if (checkVecOutside(parWorld, parPos, vecTry)) return true;

        vecTry = Vec3.createVectorHelper(
            parPos.xCoord + ForgeDirection.SOUTH.offsetX * rangeCheck,
            parPos.yCoord + yOffset,
            parPos.zCoord + ForgeDirection.SOUTH.offsetZ * rangeCheck);
        if (checkVecOutside(parWorld, parPos, vecTry)) return true;

        vecTry = Vec3.createVectorHelper(
            parPos.xCoord + ForgeDirection.EAST.offsetX * rangeCheck,
            parPos.yCoord + yOffset,
            parPos.zCoord + ForgeDirection.EAST.offsetZ * rangeCheck);
        if (checkVecOutside(parWorld, parPos, vecTry)) return true;

        vecTry = Vec3.createVectorHelper(
            parPos.xCoord + ForgeDirection.WEST.offsetX * rangeCheck,
            parPos.yCoord + yOffset,
            parPos.zCoord + ForgeDirection.WEST.offsetZ * rangeCheck);
        if (checkVecOutside(parWorld, parPos, vecTry)) return true;
    ...
    public static boolean checkVecOutside(World parWorld, Vec3 parPos, Vec3 parCheckPos) {
        ...
        if (dirNorth) {
            if (parWorld.getHeightValue(
                MathHelper.floor_double(parCheckPos.xCoord),
                MathHelper.floor_double(parCheckPos.zCoord)) < parCheckPos.yCoord) return true;
        }
        return false;
    }
}
