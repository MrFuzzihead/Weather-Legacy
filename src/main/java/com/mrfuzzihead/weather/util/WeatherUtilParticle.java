...
public class WeatherUtilParticle {
    ...
    static {
        ...
        for (int i = 0; i < maxRainDrops; i++) {
            rainPositions[i] = Vec3.createVectorHelper(
                (rand.nextFloat() * range) - (range / 2),
                (rand.nextFloat() * range / 16) - (range / 32),
                (rand.nextFloat() * range) - (range / 2));
        }
    }
    ...
    public static void getFXLayers() {
        ...
        try {
            field = (EffectRenderer.class).getDeclaredField("field_78876_b");// ObfuscationReflectionHelper.remapFieldNames("net.minecraft.client.particle.EffectRenderer",
                                                                             // new String[] { "fxLayers" })[0]);
            field.setAccessible(true);
            fxLayers = (List[]) field.get(
     ...
     * {
     * EntityRotFX var31 = new EntityTexBiomeColorFX(worldRef, (double)xx, (double)yy - 0.5, (double)zz, 0D, 0D, 0D,
     * 10D, 0, effLeafID, id, getBlockMetadata(xx, yy, zz), xx, yy, zz);
     ...
     * EntityRotFX var31 = new EntityTexFX(worldRef, (double)xx, (double)yy + 0.5, (double)zz, 0D, 0D, 0D, 10D, 0,
     * effWind2ID);
     ...
     * if (getBlockId(xx, yy + 1, zz) == 0)
     * {
     * EntityTexFX var31 = new EntityTexFX(worldRef, (double)xx, (double)yy + 0.5, (double)zz, 0D, 0D, 0D, 10D, 0,
     * effSandID);
