...
import com.mrfuzzihead.weather.CommonProxy;
...
public class WeatherUtil {
            ...
            if (ConfigMisc.Storm_Tornado_GrabCond_List) {
                ...
                if (ConfigMisc.Storm_Tornado_GrabCond_StrengthGrabbing) {
                    ...
                    if (block == null) {
                        ...
                        float strVsBlock = block.getBlockHardness(parWorld, 0, 0, 0)
                            - (((itemStr.func_150997_a(block) - 1) / 4F));
                ...
                if (ConfigMisc.Storm_Tornado_RefinedGrabRules) {
                    if (id == Blocks.dirt || id == Blocks.grass
                        || id == Blocks.sand
                        || block instanceof BlockLog/* || block.blockMaterial == Material.wood */) {
                        result = false;
                    }
                }
            }
    ...
    public static boolean safetyCheck(Block id) {
        if (id != Blocks.bedrock && id != Blocks.log
            && id != Blocks.chest
            && id != Blocks.jukebox/* && id != Block.waterMoving.blockID && id != Block.waterStill.blockID */) {
            return true;
        } else {
            return false;
        }
